...
const AnalysisCard: React.FC<AnalysisCardProps> = ({ analysis, isLoading, error, onAnalyze, disabled = false }) => {
    ...
    return (
        <div className="bg-gray-800/50 p-6 rounded-lg shadow-lg border border-gray-700">
            ...
            {!analysis && !isLoading && !error && (
                 <div className="text-center py-8 text-gray-400 border-t border-gray-700 mt-4">
                    {disabled 
                        ? "Upload a CSV file with team data to enable analysis."
                        : 'Adjust the instructions if needed and click "Analyze Names" to generate insights.'}
                 </div>
            )}
